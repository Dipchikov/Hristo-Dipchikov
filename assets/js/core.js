...
$(document).ready(function(){
	...
	$("div#sidebar div.sidebar div.item").hoverIntent({    
		over: function () {
	    	var el = $(this);
	    	el.children("a").fadeTo(100,1); 
	    	el.children("div.item_bg").animate({ opacity: 0 }, 800);
	  	},
	    timeout: 150, 
	    out: function () {
	    	$(this).children("div.item_bg").animate({ opacity: 0.5 }, 400);	 
	    	$(this).children("a").fadeTo(400,0.4);    	
	  	} 
	});
		...
		if(window.location.hash) {
			...
			if (nls_hash.indexOf("#!") != -1) {
				...
				if ( !isNaN( nls_hash.replace(/\//g,"") ) ){
					...
					content.url = "/"+nls_hash[0]+"/index.json";
					
					if (nls_hash[0] == 'prints' && nls_hash[1]){
						content.url = "/"+nls_hash[0]+"/"+nls_hash[1]+"/index.json";
					}
					...
					setTimeout(function() {
						$.ajax({
							...
							success: function (results) {
								...
								if (nls_hash[0] !== 'home'){
									document.title = results.title_raw + " » Фотография от Христо Дипчиков на Деца, Семейства, Абитуриенти, Природа и Архитектура ";
								} else {
									document.title ="Фотография от Христо Дипчиков на Деца, Семейство, Абитуриенти, Природа и Архитектура";
								}	
